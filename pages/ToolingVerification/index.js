...
import {
	LoadPDATongsValidationList,
	AuditTongsValidationData,
	DeletePDATongsValidationData
...
export default {
	...
	methods: {
		...
		// 获取列表数据
		async getLoadData() {
			const res = await LoadPDATongsValidationList(this.listQuery)
			const data = res.Result
		...
		detailedClick(item) {
			...
			if (item.CHECK_CODE) {
				uni.navigateTo({
					url: '/pages/ToolingVerification/details?CODE=' + item.CHECK_CODE + '&ID=' + item.ID
				})
			} else {
				if(start){
					uni.showModal({
				...
				uni.navigateTo({
					url: '/pages/ToolingVerification/details'
				})
			}
		...
		async deleteClick(row) {
			uni.showModal({
				...
				success: (e) => {
					if (e.confirm) {
						DeletePDATongsValidationData(row.CHECK_CODE).then(res => {
							if (res.Result) {
								this.$voice.success()
								uni.showModal({
									title: '提示',
		...
		async reviewSub() {
			...
			const res = await AuditTongsValidationData(ReviewData)
			if (res.Result) {
				this.$voice.success()
				uni.showModal({
					title: '提示',
