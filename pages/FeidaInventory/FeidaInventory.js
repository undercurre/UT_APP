...
import {
	...
	LoadPDAFeederCheckList,
	DeletePDAFeederCheckData
...
export default {
	...
	methods: {
		...
		// 获取列表数据
		async getLoadData() {
			const res = await LoadPDAFeederCheckList(this.listQuery)
			const data = res.Result
		...
		detailedClick(item) {
			...
			if (item.CHECK_CODE) {
				uni.navigateTo({
					url: '/pages/FeidaInventory/details?EEDER=' + item.CHECK_CODE + '&&MSTID=' + item.ID+ '&&STATUS=' + item.CHECK_STATUS
				})
			} else {
				if (start) {
					uni.showModal({
				...
				uni.navigateTo({
					url: '/pages/FeidaInventory/details'
				})
			}
		...
		async deleteClick(row) {
			uni.showModal({
				...
				success: (e) => {
					if (e.confirm) {
						DeletePDAFeederCheckData(row.CHECK_CODE).then(res => {
							if (res.Result) {
								this.$voice.success()
								uni.showModal({
									title: '提示',
		...
		async reviewSub() {
			...
			const res = await AuditFeederCheckData(ReviewData)
			if (res.Result) {
				this.$voice.success()
				uni.showModal({
					title: '提示',
