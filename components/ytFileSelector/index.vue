<template>
	<view class="content">
		<view class="file-select-container" @click="_autoClose">
			<view class="file-select-wrap" @click.stop>
				<view class="file-select-header"><text>文件选择</text></view>
				<scroll-view class="s-f-scroll-view" scroll-y>
					<block>
						...
						<view @click.stop="showDir(item.isDirectory, item.path, item.pathName)" class="s-f-item" v-for="(item, i) in flileList" :key="i">
							<view class="s-f-item-path-name">
				...
				<view class="file-select-footer">
					<view class="s-f-btn s-f-cancel" style="color: #fff;background: #F56C6C;" @click="closeHandle">取消</view>
					<view class="s-f-btn s-f-okey" style="color: #fff;background: #409EFF;" @click="okeyHandle">确定</view>
				</view>
			</view>
		</view>
	</view>
...
export default {
	...
	methods: {
		...
		choose() {
			...
			// #endif
			this.isChoose = true;
			this.flileList = this.fileType(files.getRootDir());
		},
		backToPrevent() {
			let r = this.flag - 1;
			if (r === 0) {
				...
				this.flileList = this.fileType(files.getSubDir(this.treeList[r]));
			}
			this.flag = r;
		},
		setOrDelSelectData(path, size, pathName) {
			// 多选
		...
		showDir(isDir, path, pathName) {
			let _this = this;
			if (isDir) {
				this.treeList.push(path);
				this.flileList = this.fileType(files.getSubDir(path));
				this.flag += 1;
			} else {
				uni.getFileInfo({
					filePath: 'file:///' + path.toString(),
					success(r) {
						_this.setOrDelSelectData(path.toString(), r.size, pathName);
					},
					fail(err) {
						console.log('err:', err);
					}
		...
		fileType(value) {
			...
			value.map(item => {
				if (!item.isDirectory) {
					if (item.pathName.endWith('.mp3')) {
						arr.push(item);
					}
					if (item.pathName.endWith('.wav')) {
						arr.push(item);
					}
				} else {
					arr.push(item);
