...
// import {
// 	AuditTongsCheckData,
// 	LoadPDATongsCheckList,
// 	DeletePDATongsCheckData
// } from '@/api/MesTongsInfo.js'
import {
	AuditEquipmentCheckData,
	LoadPDAEquipmentCheckList,
	DeletePDAEquipmentCheckData
...
export default {
	...
	methods: {
		...
		// 获取列表数据
		async getLoadData() {
			const res = await LoadPDAEquipmentCheckList(this.listQuery)
			const data = res.Result
		...
		detailedClick(item) {
			...
			if (item.CHECK_CODE) {
				uni.navigateTo({
					url: '/pages/EquipmentInventory/maintain?CHECK_CODE=' + item.CHECK_CODE + '&&MSTID=' + item.ID
				})
			} else {
				if(start){
					uni.showModal({
				...
				uni.navigateTo({
					url: '/pages/EquipmentInventory/maintain'
				})
			}
		},
		// 删除
		async deleteClick(row) {
			uni.showModal({
				...
				success: (e) => {
					if (e.confirm) {
						DeletePDAEquipmentCheckData(row.CHECK_CODE).then(res => {
							if (res.Result) {
								this.$voice.success()
								uni.showModal({
									title: '提示',
		...
		async reviewSub() {
			...
			const res = await AuditEquipmentCheckData(ReviewData)
			if (res.Result) {
				this.ReviewForm = null
				this.$voice.success()
				uni.showModal({
