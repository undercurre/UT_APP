...
export default {
	...
	methods: {
		...
		async EnterCheckRee(e) {
			...
			console.log(this.enterVal)
			this.SiteObj.carton.DefinedQty = this.DefinedQty
			...
			if (data) {
				...
				if (data.carton) {
					this.Carton_NO = data.carton.Carton_NO
					if(!this.DefinedQty){
						this.DefinedQty = data.carton.DefinedQty
					}
					
					this.CurrentQty = data.carton.CurrentQty
				}
				// 打印条件
				if (data.printer) {
		...
		async FullClick() {
			...
			this.SiteObj.carton.DefinedQty = this.DefinedQty
			this.SiteObj.data = this.enterVal
			const res = await SetCatonFull(this.SiteObj)
