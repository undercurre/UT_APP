...
export default {
	...
	methods: {
		...
		// 获取列表数据
		async getLoadData() {
			const res = await LoadPDATongsCheckList(this.listQuery)
			const data = res.Result
		...
		detailedClick(item) {
			...
			if (item.CHECK_CODE) {
				uni.navigateTo({
					url: '/pages/ToolingInventory/details?EEDER=' + item.CHECK_CODE + '&&MSTID=' + item.ID+ '&&STATUS=' + item.CHECK_STATUS
				})
			} else {
				if(start){
					uni.showModal({
				...
				uni.navigateTo({
					url: '/pages/ToolingInventory/details'
				})
			}
		...
		async deleteClick(row) {
			uni.showModal({
				...
				success: (e) => {
					if (e.confirm) {
						DeletePDATongsCheckData(row.CHECK_CODE).then(res => {
							if (res.Result) {
								this.$voice.success()
								uni.showModal({
									title: '提示',
		...
		async reviewSub() {
			...
			const res = await AuditTongsCheckData(ReviewData)
			if (res.Result) {
				this.ReviewForm = null
				this.$voice.success()
				uni.showModal({
